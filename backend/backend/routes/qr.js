  ...
  try {
    ...
    if (activeCampuses.length > 0) {
      ...
      for (const campus of activeCampuses) {
        if (campus.isPointInside(location.latitude, location.longitude)) {
          isInsideCampus = true;
      ...
      if (!isInsideCampus) {
        return res.status(400).json({
          success: false,
          message: `You are outside the campus area. Please move to the campus to mark attendance.`,
          errorType: 'CAMPUS_GEOFENCE_FAILED'
        });
      }
    }
